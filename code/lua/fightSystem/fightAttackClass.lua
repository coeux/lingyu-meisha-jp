--fightAttackClass.lua
--=======================================================================
--一次攻击的类
FightAttackClass = class();

function FightAttackClass:constructor(id, attacker, targeterList, attackerSkill)
  --[[local str = "[" .. tostring(attackerSkill:GetScriptName()).. "] ATTACKER: " .. tostring(attacker.resID) ;
  if attacker.m_isEnemy then
    str = str .. " <== "; 
  else
    str = str .. " ==> "; 
  end
...
function FightAttackClass:CreateSkillScript()
  ...
  if self.m_targeterList.buffTargeterList ~= nil and 
     self.m_attackerSkill:GetPeriod2() == ReleasePeriod.instantaneous and 
     self.m_attackerSkill:HaveBuffCanReleaseToAlly() then
      for _, targeter in ipairs(self.m_targeterList.buffTargeterList) do
        --
        if math.random(1, 100) <= CaculateSkillAccuracyProbability(self.m_attacker, targeter, self.m_attackerSkill) * 100 then
          __.each(self.m_attackerSkill:GetBuff2ID(), function(buff)
            if math.random(1, 100) <= buff.pro * 100 then
              targeter:AddBuff(buff.id, self.m_attacker, self.m_attackerSkill, false, buff.level);
            end
          end)
        end
      end
...
--创建手势技能脚本
function FightAttackClass:CreateGestureScript(skillID, scriptName)
...
function FightAttackClass:Destroy()
  ...
  if (self.m_scriptID ~= -1) then
    effectScriptManager:DestroyEffectScript(self.m_scriptID);
  end  
...
--打断技能
function FightAttackClass:Interrupt()
  if (self.m_scriptID ~= -1) and (effectScriptManager:GetEffectScript(self.m_scriptID) ~= nil) then
      effectScriptManager:GetEffectScript(self.m_scriptID):Interrupt();
    end
end
...
--重新寻找己方目标
function FightAttackClass:RefindSkillAllyTargeters()
  local targeterList = self.m_attackerSkill:RefindSkillAllyTargeters(self.m_targeterList.buffTargeterList);

  if targeterList ~= nil then
    self.m_targeterList.buffTargeterList = targeterList;
  else
    self.m_targeterList.buffTargeterList = nil;
  end
...
--获取友军目标
function FightAttackClass:GetAllyTargeterList()
  return self.m_targeterList.buffTargeterList;
end
...
--设置手势技能ID
function FightAttackClass:SetGestureSkill(skillID, scriptName, isRecoverHp)
  self.m_gestureSkillID = skillID;
  self.m_isRecoverHP = isRecoverHp;
  self:CreateGestureScript(skillID, scriptName);
end

--获取手势技能ID
function FightAttackClass:GetGestureSkillID()
  return self.m_gestureSkillID;
end

--是否回血
function FightAttackClass:IsRecoverHP()
  return self.m_isRecoverHP;
end
