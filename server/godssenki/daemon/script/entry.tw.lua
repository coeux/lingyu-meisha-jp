...
--local mailaddr = 'http://server.cszs.appget.cn/godtool/server/mail.php'
local mailaddr = 'http://tw.god.123u.com/godtool/server/mail.php'
...
function start_prog(v)
    os.execute(string.format("sudo /data/app/startup start %s %s %d", v.domain, v.progname, tonumber(v.hostnum)));
    return 'ok';
end

function stop_prog(v)
    os.execute(string.format("sudo /data/app/startup stop %s %s %d", v.domain, v.progname, tonumber(v.hostnum)));
    return 'ok';
end
...
call['start_prog']=start_prog
call['stop_prog']=stop_prog
