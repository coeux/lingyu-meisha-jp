...
echo $var
ID=`ps -ef | grep "server5" | grep -v "$0" | grep -v "grep" | awk '{print $2}'`
IDNAME=`ps -ef | grep "server5" | grep -v "$0" | grep -v "grep" | awk '{print $8}'`
...
then
    ...
    do
        ...
        if [ "$var" = "scene" -a "$1" = "slog" ]; then
            ./$var/$var --server5 -id 1 -conf ./config-server5.lua
        elif [ "$var" = "scene" -a "$1" = "sgdb" ]; then
            gdb -ex "set args --server5 -id 1 -conf ./config-server5.lua" ./scene/scene
        else
            ./$var/$var --server5 -id 1 -conf ./config-server5.lua -d
            ps -aux | grep $var
        fi
    done
fi
