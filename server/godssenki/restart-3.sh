...
echo $var
ID=`ps -ef | grep "server3" | grep -v "$0" | grep -v "grep" | awk '{print $2}'`
IDNAME=`ps -ef | grep "server3" | grep -v "$0" | grep -v "grep" | awk '{print $8}'`
...
then
    ...
    do
        ...
        if [ "$var" = "scene" -a "$1" = "slog" ]; then
            ./$var/$var --server3 -id 1 -conf ./config-server3.lua
        elif [ "$var" = "scene" -a "$1" = "sgdb" ]; then
            gdb -ex "set args --server3 -id 1 -conf ./config-server3.lua" ./scene/scene
        else
            ./$var/$var --server3 -id 1 -conf ./config-server3.lua -d
            ps -aux | grep $var
        fi
    done
fi
