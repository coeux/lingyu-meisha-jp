...
echo $var
ID=`ps -ef | grep "server2" | grep -v "$0" | grep -v "grep" | awk '{print $2}'`
IDNAME=`ps -ef | grep "server2" | grep -v "$0" | grep -v "grep" | awk '{print $8}'`
...
then
    ...
    do
        ...
        #./$var/$var --server2 -id 1 -conf ./config-server2.lua -d
        #ps -aux | grep $var
        if [ "$var" = "scene" -a "$1" = "slog" ]; then
            ./$var/$var --server2 -id 1 -conf ./config-server2.lua
        elif [ "$var" = "scene" -a "$1" = "sgdb" ]; then
            gdb -ex "set args --server2 -id 1 -conf ./config-server2.lua" ./scene/scene
        else
            ./$var/$var --server2 -id 1 -conf ./config-server2.lua -d
            ps -aux | grep $var
        fi
