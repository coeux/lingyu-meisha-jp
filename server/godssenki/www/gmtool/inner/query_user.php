...
function show_user_table($result_)
{
    ...
    {
        $query = 'select name from Account where aid='.$row['aid'];
        ...
        show_table_row_submit($infos, 6, array(
...
function search_user($name)
{
    ...
    $query = '';
    if (strlen($name) == 0){
    	$query = "select aid,uid,nickname,resid,grade from UserInfo where nickname like '$username' limit 30";
    }else{
