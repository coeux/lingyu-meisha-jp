...
function show_partner_table($result_)
{
    ...
    {
        show_table_row_partner($row, 7, 'chk_pid');
    }

    end_table();
}

function show_table_row_partner1($row_, $n_, $check_name_)
{
	echo '<tr> ';
	echo '</tr>';
}
...
function show_table_row_partner($row_, $n_, $check_name_)
{
    ...
    for($i=0;$i<$n_;$i++)
    {
        ...
        elseif ($i == 2)
        {
            ...
            $role = $rp_role[$row_[1]];
            echo format('<td align="center"><input type="text" name="{0}" value="{1}"/></td>', 'name', $role->name);
        }
        elseif ($i == 3)
        {
            echo format('<td align="center"><input type="text" name="{0}" value="{1}"/></td>', 'grade', $row_[$i-1]);
        }
        elseif ($i == 4)
        {
            echo format('<td align="center"><input type="text" name="{0}" value="{1}"/></td>', 'exp', $row_[$i-1]);
        }
        elseif ($i == 5)
        {
            echo format('<td align="center"><input type="text" name="{0}" value="{1}"/></td>', 'quality', $row_[$i-1]);
        }
        elseif ($i == 6)
        {
            echo format('<td align="center"><input type="text" name="{0}" value="{1}"/></td>', 'potential', $row_[$i-1]);
        }
    }
