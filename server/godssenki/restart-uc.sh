...
echo $var
ID=`ps -ef | grep "server_uc" | grep -v "$0" | grep -v "grep" | awk '{print $2}'`
IDNAME=`ps -ef | grep "server_uc" | grep -v "$0" | grep -v "grep" | awk '{print $8}'`
...
then
    ...
    do
        ...
        if [ "$var" = "scene" -a "$1" = "slog" ]; then
            ./$var/$var --server_uc -id 1 -conf ./config-server_uc.lua
        elif [ "$var" = "scene" -a "$1" = "sgdb" ]; then
            gdb -ex "set args --server_uc -id 1 -conf ./config-server_uc.lua" ./scene/scene
        else
            ./$var/$var --server_uc -id 1 -conf ./config-server_uc.lua -d
            ps -aux | grep $var
        fi
    done
fi
