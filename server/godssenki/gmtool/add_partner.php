...
<body>
<form method="get" action="fix_partner.php" >
<div align="center">职业
...
<?php
    echo format('<input type="hidden" name="uid" value="{0}">', $_GET['uid']);
?>
<input type="submit" name="filter_partner" value="过滤"/>
</div>
</form>
<form  name="new_partner_form" method="get" action="fix_partner.php" onSubmit="return get_jparam()">
<?php
    echo format('<input type="hidden" name="uid" value="{0}">', $_GET['uid']);

    if (!empty($_GET['filter_partner']))
        show_partner_res_table($_GET['sel_partner_job'], 
            $_GET['sel_partner_rare']);
    else
